...
class User < ActiveRecord::Base
  ...
  def increment(counter, mobile = false)
    if counter == :correct_count && !mobile
  ...
  def add_answer_for_project(answer, project, referer = nil, mobile = false)
    if valid_answer?(answer) and valid_project?(project)
      answers << answer
      increment_counter project, answer.counter_name, referer, mobile
  ...
  def increment_counter project, counter, referer = nil, mobile = false
      participation = Participation.get_participation(self, project, referer)
      increment(counter,mobile)
