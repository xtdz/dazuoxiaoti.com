...
window.weibo_button = (id, content, path, image_path, size) ->
  _w = 19
  _h = 16
  type = '3'
  if (size == 'big')
...
window.renren_button = (id, content, path, image_path, size) ->
...
window.qq_button = (id, content, path, image_path, size) ->
  param = {
    url: 'http://' + location.host + path,
    appkey: '801076262',
